...
DtofRec::DtofRec(string in, string out, TTree *tree) : CRdata(in, out, tree),
...
void DtofRec::MirrorTransformation(DtofRec::lightpath a, int i)
{
  if (a == direct)
    return;
  if (a == left)
...
void DtofRec::Loop()
{
  ...
  for (Long64_t tentry = 0; tentry < nentries; tentry++)
  {
    ...
    TT.push_back(Mean(16*sensorN, FT)*16*sensorN/ctr);
    AA.push_back(Mean(16*sensorN, TL)*16*sensorN/ctr);
    ot->Fill();
    exctr++;
    }
  }

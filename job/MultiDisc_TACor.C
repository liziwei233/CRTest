...
void MultiDisc_TACor(const char *rootname=""){
    ...
    //return;
        ...
        TH1D *h[20] = new TH1D("h",";Time (s);Counts",bint,tL,tR);
        TH1D *hU[20] = new TH1D("hU",";Amplitude (mV);Counts",binu,uL,uR);
        TH1D *hpfx[20];        
        TH2D *ht[20] = new TH2D("ht",";Amp (mV);Time (s)",binu,uL,uR,bint,tL,tR);
        TF1 *fitAT[20] = new TF1("fitAT","0",uL,uR);
        //t->Draw("T0>>h");
        for(int j=0;j<T;j++){

        TH1D *h[j] = new TH1D("h",";Time (s);Counts",bint,tL,tR);
        TH1D *hU[j] = new TH1D("hU",";Amplitude (mV);Counts",binu,uL,uR);
        TH1D *hpfx[j];        
        TH2D *ht[j] = new TH2D("ht",";Amp (mV);Time (s)",binu,uL,uR,bint,tL,tR);
        TF1 *fitAT[j] = new TF1("fitAT","0",uL,uR);
        }
            int N = 0;
            N = t->GetEntries();
        ...
        for(int i=0;i<N;i++){
                ...
                T0L_cor = fitAT->Eval(ULtemp);
                T0R_cor = fitAT->Eval(URtemp);
